"""Tests for the `date-extract` command and its worker."""
...
from rematter import _date_extract_worker, app
...
def test_datetime_field_uses_date_part(tmp_path: Path) -> None:
    ...
    status, msg = _date_extract_worker(f, field="created", dry_run=False)
    assert status == "done"
...
def test_date_field_string(tmp_path: Path) -> None:
    ...
    f = tmp_path / "note.md"
    f.write_text('---\nDate: "2026-01-15"\n---\nBody.\n')
    status, _ = _date_extract_worker(f, field="Date", dry_run=False)
    assert status == "done"
...
def test_already_prefixed_is_skipped(tmp_path: Path) -> None:
    ...
    f.write_text("---\ncreated: 2026-01-01\n---\nBody.\n")
    status, _ = _date_extract_worker(f, field="created", dry_run=False)
    assert status == "skip"
...
def test_invalid_date_string_returns_error(tmp_path: Path) -> None:
    f = tmp_path / "note.md"
    f.write_text('---\nDate: "not-a-date"\n---\nBody.\n')
    status, msg = _date_extract_worker(f, field="Date", dry_run=False)
    assert status == "error"
...
def test_dry_run_makes_no_changes(tmp_path: Path) -> None:
    ...
    status, _ = _date_extract_worker(f, field="created", dry_run=True)
...
def test_field_stripped_from_frontmatter(tmp_path: Path) -> None:
    ...
    _date_extract_worker(f, field="created", dry_run=False)
...
def test_cli_date_extract_skips_already_prefixed(mock_source: Path) -> None:
